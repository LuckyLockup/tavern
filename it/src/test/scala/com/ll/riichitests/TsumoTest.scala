  ...
  "Declaring tsumo" in new CommonData {
    ...
    val tileFromTheWall = player1.ws.expectWsMsg {
      case fromTheWall: WsMsgOut.Riichi.TileFromWallTaken =>
        ...
        fromTheWall.commands.head should be(RiichiWsCmdWs.DeclareTsumo(tableId, gameId, Some(HandValue(1, 1))))
        fromTheWall
    }
    player1.ws ! tileFromTheWall.commands.head.asInstanceOf[RiichiWsCmdWs.DeclareTsumo].copy(approxHandValue = None)
